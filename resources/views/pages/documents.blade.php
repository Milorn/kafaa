<x-layouts.main>
    ...
    <x-slot:main>
        ...
        <form action="{{route('documents')}}" method="GET" class="flex  max-w-screen-lg mx-auto my-12">
            <input type="text" name="search" value="{{request()->query('search')}}" class="w-full  bg-[#EDEDED] py-2 px-4 border-2 border-[#DEDEDE] border-r-0 rounded-tl-md rounded-bl-md hover:outline-none focus:outline-none" placeholder="Trouvez un article qui vous intéresse">
        ...
        <div class="grid grid-cols-3 gap-x-5 gap-y-6 max-w-screen-xl mx-auto px-12">
            @foreach ($documents as $document)
                <div class="rounded-3xl shadow-[2px_15px_12px_0px_rgba(0,0,0,0.25)]">
                    ...
                    src="{{$post->getFirstMediaUrl("documents") ? $post->getFirstMediaUrl("documents") : asset('images/placeholder.webp') }}">
